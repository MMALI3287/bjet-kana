...
const Stats = () => {
  ...
  const resumeStopwatch = useStatsStore(state => state.resumeStopwatch);
  ...
  const statsFields = [
    {
      field: 'training time',
      value: `${totalMinutes} minute${totalMinutes === 1 ? '' : 's'
    ...
    },
    {
      ...
      value: `${averageCorrectAnswerTime
...

    {
      ...
      value: `${fastestCorrectAnswer !== 'Infinity' ? fastestCorrectAnswer + 's' : '~'
        }`,
      icons: [Flame],
    },
    {
      field: 'slowest correct answer',
      value: `${slowestCorrectAnswer !== '-Infinity' ? slowestCorrectAnswer + 's' : '~'
        }`,
      icons: [ClockFading],
    },
    {
      field: 'correct / incorrect answers ratio',
      value: `${Number.isNaN(ciRatio)
          ? '~'
          : ciRatio === Infinity
            ? '∞'
            : ciRatio.toFixed(2)
